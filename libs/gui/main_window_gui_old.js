...
module.exports = {
	...
	init: function() {
		...
		var powerColumns = [
			{id: 'powers1',     e: null, syncing: false, parent: null},
			{id: 'powers2',     e: null, syncing: false, parent: null},
			{id: 'powers3',     e: null, syncing: false, parent: null},
			{id: 'scrolldummy', e: null, syncing: false, parent: null}
		];
		...
		var onscrollHandler = function() {
			if (!this.syncing)
			{
				...
				{
					if (this.parent[i].id == this.id) continue;

					this.parent[i].syncing     = true;
					this.parent[i].e.scrollTop = this.e.scrollTop;
				}
			}
		};
		...
		var h = 0;
		var i;
		for (i=0 ; i<powerColumns.length ; i++)
		{
			powerColumns[i].e                = document.getElementById(powerColumns[i].id);
			powerColumns[i].e.onscroll       = onscrollHandler;
			powerColumns[i].parent           = powerColumns;
			powerColumns[powerColumns[i].id] = powerColumns[i];
		}

		for (i=0 ; i<powerColumns.length ; i++)
		{
			if (powerColumns[i].id == 'scrolldummy') continue;
			if (powerColumns[i].e.clientHeight > h) h = powerColumns[i].e.clientHeight;
		}

		powerColumns['scrolldummy'].e.style.height = h + 'px';
	},

	enhCheckMode: false,
	catalystMode: false,
	boostersMode: false,
	updateUIFromToggles: function() {
		// enhCheckmode
		document.getElementById('senhchecktoggle').className = 'heroaction' + (this.enhCheckMode ? ' active' : '');
		document.getElementById('powers1').className = 'col-md-3 powerslots powers1' + (this.enhCheckMode ? ' enhcheck' : '');
		document.getElementById('powers2').className = 'col-md-3 powerslots powers2' + (this.enhCheckMode ? ' enhcheck' : '');
		document.getElementById('powers3').className = 'col-md-3 powerslots powers3' + (this.enhCheckMode ? ' enhcheck' : '');

		// catalystMode
		document.getElementById('scatalysttoggle').className = 'heroaction' + (this.catalystMode ? ' active' : '');

		// boostersMode
		document.getElementById('sboostertoggle').className = 'heroaction' + (this.boostersMode ? ' active' : '');
	},

	addCSSClass: function(baseClasses, className, condition=true, defaultValue='') {
	...
	removeCSSClass: function(baseClasses, className, condition=true) {
		var chunks = baseClasses.trim().split(/\s/g);
		var i;
	...
	addRemoveCSSClass: function(baseClasses, className, condition=true) {
		if (condition)
		{
	...
	getBgImages: function(id, prefix=false) {
		try
		{
	...
	_splitBgImagesList: function(bgList) {
	...
	addBgImage: function(bgList, newImg, imgType, prefix=false) {
	...
	removeBgImageByType(bgList, imgType, prefix=false) {
		var urlList = this._splitBgImagesList(bgList);
		var tmp = [], i, type;
	...
	replaceBgImageByType(bgList, imgType, imgUrl, prefix=false) {
	...
	infoPopup: {
		...
		trigger: function(o) {
			...
			var ret = '';
			ret += '<h1 class="' + rarityClass + '">' + longname.replace(/\s/g, '&nbsp;') + '</h1>';
			for (i=0 ; i<enhIDList.length ; i++)
			{
	...
	loadBuild: function(file) {
		...
		for (i=0 ; i<charBuild.getPowersCount() ; i++)
		{
			...
			switch (p.name)
			{
				...
				default:
					if (/^(Bright|Dark) Nova (.+)/.test(p.name))
					{
						ci = 1;
					}
					else if (/^(White|Black) Dwarf (.+)/.test(p.name))
					{
						ci = 2;
					}
			}
